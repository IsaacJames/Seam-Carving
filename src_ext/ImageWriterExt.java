...
public class ImageWriterExt {
    ...
    // duplicates seam in pixel array
    public static int[][][] duplicatePixelSeam(int[][][] pixels, int[][] path) throws IOException {
        // create a new array with dimensions corresponding to an image with a width increased by 1 pixel
        int[][][] newImage = new int[pixels.length][pixels[0].length + 1][3];
        ...
        for (int y = 0; y < pixels.length; y++) {
            ...
            for(int x = 0; x <= path[path.length - 1 - y][0] - 1 && x != pixels[0].length; x++) {
                newImage[y][x] = pixels[y][x];
            }
            // after the x coordinate of the path is reached add the corresponding pixel again, then continue
            // copying the old array (effectively duplicating the pixel)
            for (int x = path[path.length - 1 - y][0] - 1; x < pixels[0].length; x++) {
                newImage[y][x + 1] = pixels[y][x];
            }
        }
        return newImage;
    }
